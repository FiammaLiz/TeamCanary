%% Figura consolidacion curvas separadas
%Figura con curvas de suavizado mas desvio estandard de los tres estados
...
%Envolvente de los oscilogramas normalizados
    ...
    hold on
    plot(times,averages(fil,:)+stdevs(fil,:),'LineWidth',1,'Color', [0,0.3882,0],'LineStyle','-.')
    plot(times,averages(fil,:)-stdevs(fil,:),'LineWidth',1,'Color', [0,0.3882,0],'LineStyle','-.')
    line(list_anchors'*[1 1],j(2).YLim,'color',[0.5 0.5 0.5 0.5]); %lineas grises para delimitar onsets y offsets 
    ylabel({'Curva de suavizado'; 'Cantando'},'Color',[0,0.3882,0]);
    hold off
    
    %Curva de suavizado con desvio, anestesiado
    j(3)=subplot(4,1,3);
    plot(xi,hbcc_m,'Color','r','LineWidth',1); 
    hold on
    %inserte aqui desvio
    plot(xi,hbcc_m+hbcc_std,'Color','r','LineWidth',1,'LineStyle','-.'); 
    plot(xi,hbcc_m-hbcc_std,'Color','r','LineWidth',1,'LineStyle','-.');
    line(list_anchors'*[1 1],j(3).YLim,'color',[0.5 0.5 0.5 0.5]); %lineas grises para delimitar onsets y offsets 
    ylabel({'Curva de suavizado'; 'Anestesiado'},'Color','r');
    
    %Curva de suavizado con desvio, vigilia
    j(4)=subplot(4,1,4);
    plot(tt,cs,'Color','b','LineWidth',1); 
    hold on
    plot(tt,cs+cs_std,'Color','b','LineWidth',1,'LineStyle','-.'); 
    plot(tt,cs-cs_std,'Color','b','LineWidth',1,'LineStyle','-.'); 
    line(list_anchors'*[1 1],j(4).YLim,'color',[0.5 0.5 0.5 0.5]); %lineas grises para delimitar onsets y offsets 
    hold off
    ylabel({'Curva de suavizado'; 'Vigilia'},'Color','b');
